...
// ------------------------------------------------------
{
   ...
   for (i in 1:nteeth){
      ...
      loghazard[i] = status_i * (log(xi) + (xi-1)*log(time_i) + 
        ri[IDi] + gamma[1] + gamma[2]*X1_i + gamma[3]*X2_i + gamma[4]*X3_i + gamma[5]*X4_i + gamma[6]*X5_i + alpha1 * linpred1_at_event_i + alpha2 * linpred2_at_event_i);
      ...
      real logsurv2 = exp(ri[IDi] + gamma[1] + gamma[2]*X1_i + gamma[3]*X2_i + gamma[4]*X3_i + gamma[5]*X4_i + gamma[6]*X5_i + 
...
// ------------------------------------------------------
   ...
   target += normal_lpdf(beta1 | 0, 2.5);
   target += normal_lpdf(beta2 | 0, 2.5);
   target += inv_gamma_lpdf(xi | 1, 1);


   // Survival fixed effects
   target += normal_lpdf(gamma | 0, 2.5);

   // Association parameters
   target += normal_lpdf(alpha1 | 0, 2.5);
   target += normal_lpdf(alpha2 | 0, 2.5);
   
   // Random-effects
//   for(i in 1:nid){target += multi_normal_lpdf(bi[i,1:2] | rep_vector(0,2), Var_bi);}

//   for(i in 1:nteeth){ target += multi_normal_lpdf(bijk[i,1:4] | rep_vector(0,4), Var_bijk); }
...
//   L_bi ~ lkj_corr_cholesky(1);
//   L_bij ~ lkj_corr_cholesky(1);
  
  // Priors on standard deviations
   target += inv_gamma_lpdf(sigma_bi | 1, 1);
   target += inv_gamma_lpdf(sigma_bij | 1, 1);
   target += inv_gamma_lpdf(sigma_e | 1, 1);
   target += inv_gamma_lpdf(sigma_ri | 1, 1);

  
//   sigma_bi ~ normal(0, 1);  
//   sigma_bij ~ normal(0, 1);
//   sigma_ri ~ normal(0, 1);
//   sigma_e ~ normal(0, 1);
}
